...
import { fetchProfileDetails, fetchProfiles } from "../redux/actions/profileActions";
const ProfileDetails = () => {
  const { id } = useParams()
  ...
  useEffect(() => {
      ...
      // filterFirstThreeGirls()
      // filterFirstThreeBoys()
  }, [dispatch])
  ...
  function filterFirstThreeBoys(profiles) {
    return profiles.filter(profile => profile.profileType === "Boy").slice(0, 3);
}

// Function to filter first 3 profiles with profileType "Girl"
function filterFirstThreeGirls(profiles) {
    return profiles.filter(profile => profile.profileType === "Girl").slice(0, 3);
}
    ...
    <div className="bg-gray-100 min-h-screen py-10 px-4">
      <div className="max-w-6xl mx-auto bg-white shadow-xl rounded-lg overflow-hidden">
        ...
        {/* <div className="relative bg-gradient-to-r from-pink-500 to-purple-500 h-56">
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center text-white">
              <h1 className="text-4xl font-bold mt-10">{profile.fullName}</h1>
              <p className="text-lg mt-2">Perfect</p>
              <span className="inline-flex items-center px-3 py-1 bg-green-600 text-white rounded-full text-sm mt-2">
                <span className="mr-2 w-2 h-2 bg-green-400 rounded-full"></span>
                Online Now
          ...
          <img
            src={profile.image}
            alt="Profile"
            className="absolute bottom-[-50px] object-cover left-8 w-60 h-60 rounded-full border-4 border-white shadow-md"
          />
        </div> */}
        <div className="relative bg-gradient-to-r from-pink-500 to-purple-500 h-56 sm:h-64 md:h-72 lg:h-80">
  <div className="absolute inset-0 flex items-center justify-center">
    <div className="text-center text-white px-4">
      <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mt-6 sm:mt-8 md:mt-10">
        {profile.fullName}
      </h1>
      <p className="text-sm sm:text-base md:text-lg mt-1 sm:mt-2">Perfect</p>
      <span className="inline-flex items-center px-3 py-1 bg-green-600 text-white rounded-full text-xs sm:text-sm mt-1 sm:mt-2">
        <span className="mr-2 w-2 h-2 bg-green-400 rounded-full"></span>
        Online Now
      </span>
    </div>
  ...
  <img
    src={profile.image}
    alt="Profile"
    className="absolute bottom-[-30px] sm:bottom-[-40px] md:bottom-[-50px] object-cover left-4 sm:left-6 md:left-8 w-28 h-28 sm:w-40 sm:h-40 md:w-60 md:h-60 rounded-full border-4 border-white shadow-md"
  />
</div>
...
<div className="flex flex-col sm:flex-row justify-between items-center mt-10 sm:mt-16 px-4 sm:px-8 space-y-4 sm:space-y-0">
  <div className="flex flex-col sm:flex-row sm:space-x-4 space-y-4 sm:space-y-0 w-full sm:w-auto">
    <button className="bg-pink-500 text-white px-4 py-2 rounded-md hover:bg-pink-600 hover:shadow-lg transition w-full sm:w-auto">
      Message Me
    </button>
    <button className="bg-red-500 text-white px-4 py-2 rounded-md hover:bg-red-600 hover:shadow-lg transition w-full sm:w-auto">
      Wink for Free
    </button>
    <button className="bg-yellow-500 text-white px-4 py-2 rounded-md hover:bg-yellow-600 hover:shadow-lg transition w-full sm:w-auto">
      Send Gift
    </button>
  </div>
  <button className="bg-gray-200 px-4 py-2 rounded-md hover:bg-gray-300 transition w-full sm:w-auto">
    Share
  </button>
</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 px-8 py-6">
          ...
          <div>
            <h2 className="text-xl font-semibold mb-4">Details</h2>
            <ul className="space-y-2 text-gray-600">
              <li>Age: <span className="font-medium">{profile.age}</span></li>
              {/* <li>Sexuality: <span className="font-medium">Straight</span></li> */}
              <li>Nationality: <span className="font-medium">Indian</span></li>
              <li>Gender: <span className="font-medium">{profile.gender}</span></li>
              <li>Caste: <span className="font-medium">{profile.caste}</span></li>
            </ul>
          </div>
...
{/* hobbies */}
<h3 className="ml-8 tracking-wider">Hobbies:</h3>
          <ul>
            {profile.hobbies.map((hobby, index) => (
              <li className="ml-12" key={index}>{hobby}</li>
            ))}
          </ul>
        {/* Recommended Profiles */}
        <div className="mt-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            { profiles.filter(profile => profile.profileType === "Girl").slice(1, 5).map((profile) => (
              <div
                key={profile.id}
                className="bg-white shadow-md rounded-lg overflow-hidden transform hover:scale-105 hover:shadow-lg transition-all duration-300"
              >
